    ...
    try:
        ...
        if sitemap_tags:
            ...
            for sitemap in sitemap_tags:
                ...
                logging.debug(f"Found nested sitemap: {sitemap_url}")
                urls.extend(parse_sitemap(sitemap_url))
            return urls

        url_tags = soup.find_all('url')
        urls = [tag.find('loc').text for tag in url_tags if is_html_page(tag.find('loc').text)]
        logging.info(f"Found {len(urls)} URLs in the sitemap")
        for u in urls:
            logging.debug(f"Found URL: {u}")
        return urls
...
def is_html_page(url):
    ...
    is_html = not any(url.lower().endswith(ext) for ext in non_html_extensions)
    logging.debug(f"URL {url} is {'an HTML page' if is_html else 'not an HTML page'}")
    return is_html
...
def extract_detailed_data(file_path, mode):
    try:
        ...
        logging.debug(f"Extracted data for {file_path}: {extracted_data}")
...
# Log the DataFrame to check extracted data
logging.info("Data extracted to DataFrame:")
